...
/// An expression can be any propositional term associated with an [super::arena::Arena].
/// However, it is not necessarily contained in the syntax tree of a formula.
/// A sub-expression, on the other hand, is a propositional term associated with a [super::formula::Formula] that actually appears in said formula's syntax tree.
...
/// An expression is always implicitly tied to an [super::arena::Arena], to which the expression's [ExprId]s or [VarId] refer.
...
/// This is sensible because any containing [super::formula::Formula], if canonical, guarantees that each of its sub-expressions is assigned exactly one identifier.
/// Thus, a shallow equality check or hash on expressions is equivalent to a deep one if they are sub-expressions of the same [super::formula::Formula].
/// While we derive [Clone], its use may violate structural sharing, which can be fixed with [super::arena::Arena::canon_visitor] if needed.
